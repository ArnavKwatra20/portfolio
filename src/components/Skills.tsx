...
export function Skills() {
  return (
    <section id="skills" aria-label="Toolkit" className="border-b border-line">
      <div className="mx-auto grid max-w-6xl gap-10 px-5 py-16 sm:px-8 md:py-20 lg:grid-cols-12">
        <div className="lg:col-span-4">
          <p className="font-mono-x text-[12px] uppercase tracking-[0.2em] text-ink-faint">04 / Toolkit</p>
        ...
        <div className="lg:col-span-8">
          <table className="w-full border-collapse border-t border-ink text-left">
            ...
            <tbody>
              {skills.map((group) => (
                <tr key={group.category} className="border-b border-line align-top">
                  <th scope="row" className="w-36 py-5 pr-6 font-mono-x text-[12px] font-medium uppercase tracking-[0.16em] text-ink-faint">
                    {group.category}
                  </th>
                  <td className="py-5 text-[15px] leading-7 text-ink">{group.items.join(', ')}</td>
                </tr>
              ))}
            </tbody>
          </table>
